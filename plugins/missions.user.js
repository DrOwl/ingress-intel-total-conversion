 ...
 *
 * Edited to work with an array of mission objects (sorting them on title)
 * for use with the iitc missions plugin
 *
 ...
 *
 */
var sort_alphanum_title = function(a, b) {
  function chunkify(t) {
    var tz = [];
    var x = 0, y = -1, n = 0, i, j;
...
window.plugin.missions = {
	...
	loadMissionsInBounds: function(bounds, callback, errorcallback) {
		...
		window.postAjax('getTopMissionsInBounds', {
			...
			missions.sort(sort_alphanum_title);
			callback(missions);
		}, function(error) {
